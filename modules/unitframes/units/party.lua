...
local Update = function(self, event, ...)
	print("party event:", event)
end

local Style = function(self, unit)
	local config = Module:GetStaticConfig("UnitFrames").visuals.units.party
	local db = Module:GetConfig("UnitFrames") 


	self:Size(unpack(config.size))
	self:Place(unpack(config.position[unit]))
	...
	self:RegisterEvent("PARTY_CONVERTED_TO_RAID", Update)
	self:RegisterEvent("GROUP_JOINED", Update)
...
UnitFrameWidget.OnEnable = function(self)
	...
	self.UnitFrame = UnitFrame:New("party1", Engine:GetFrame(), Style) 
	self.UnitFrame = UnitFrame:New("party2", Engine:GetFrame(), Style)
	self.UnitFrame = UnitFrame:New("party3", Engine:GetFrame(), Style) 
	self.UnitFrame = UnitFrame:New("party4", Engine:GetFrame(), Style) 
	-- Disable Blizzard's castbars for pet 
	self:GetHandler("BlizzardUI"):GetElement("CastBars"):Remove("party")
end
