...
Update = function(self, event, ...)
	...
	if event == "UNIT_SPELLCAST_START" then
		...
		if not CastBar.DefaultColor then
			CastBar.DefaultColor = {1, 1, 1, .25}
		end
		CastBar:SetStatusBarTexture(unpack(CastBar.DefaultColor))
		if CastBar.SwitchColor and not castdata.interrupt then
			CastBar:SetStatusBarTexture(unpack(CastBar.SwitchColor))
		end
