...
Module.UpdateZoneData = function(self)
	...
	self.data.zone_name = zoneName or ""
...
--	Minimap:Show() -- make sure the map is shown
	...
	self.frame.widgets.zone:SetFontObject(self.config.text.zone.font_object)
	self.frame.widgets.zone:SetDrawLayer("ARTWORK", 0)

	self.frame.widgets.time = self.frame.scaffold.border:CreateFontString()
	self.frame.widgets.time:SetFontObject(self.config.text.time.font_object)
	...
	self.frame.widgets.coordinates:SetFontObject(GameFontNormalHuge)
